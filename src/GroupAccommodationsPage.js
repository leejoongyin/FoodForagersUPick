...
import axios from 'axios'; // Jason api purposes
// Jason
const apiKey = 'CXMUBf-lrF2t_MhxKrxodrYQppf7BhaJJdxJSKYOfy6sSNArJSaz9VrWLxGPdssXLS98ybpYWrGhuLzN7Qy2nFmyTjJvsi3a90YO9far3aebWeDaDpn4BRxsffbGX3Yx';
...
class EatingAlone extends Component {
    ...
    // Jason hardcoded values for testing yelp
    yelpTest = () => {
        ...
        ).then((response) => this.storeData('test', response.data.businesses)); // access fields with test[0].<key>
    }
    ...
    // Jason
    // variation of getData, got rid of alert
    getDataJ = async (key) => {
        try {
    ...
    // Jason
    // used to store the data of the restaurant recommendation 
    ...
    // Jason
    // Yelp call implementation
    createRecommendation = () => {
        var byPref = this.getDataJ('byPref');
        // user wants to be recommended by preferences 
        if (byPref == true) {
            this.prefRecommendation();
        }
        // user wants to be recommended by list
        if (byPref == false) {
            this.listRecommendation();
        }
        // user has not inputted pref or restaurant list i.e. byPref == null
        else {
            this.props.navigation.navigate( // redirect them to that screen
                "Generate", 
                { isDarkmode: this.props.isDarkmode }
            );
        }
        // the methods in the if statement store the restaurant information data
        // from yelp based on list or preferences
    }

    // Jason
    // recommendation based on preferences
    prefRecommendation = () => {
        var zipcode = JSON.stringify(this.getDataJ('zipcode'));
        var budget = JSON.stringify(this.getDataJ('budget'));
        var diet = JSON.stringify(this.getDataJ('diet'));
        var cuisine = JSON.stringify(this.getDataJ('cuisine'));
        var restaurant = JSON.stringify(this.getDataJ('restaurant'));
        ...
        // diet
        for (var i in diet) {
            dietCSV = dietCSV + diet[i] + ','
        }
        // cuisine
        for (var i in cuisine) {
            cuisineCSV = cuisineCSV + cuisine[i] + ','
        }
        // restaurant type
        for (var i in restaurant) {
            restaurantCSV = restaurantCSV + restaurant[i] + ','
        }

        // prioritize diet restrictions
        if (dietCSV != '') {
            filter = dietCSV;
        }
        ...
        ).then((response) => this.storeData('recommendation', response.data.businesses)); // access fields with recommendation[0].<key>

    }

    // Jason
    // recommendation based on restaurant list
    listRecommendation = () => {
                ...
                <TouchableWithoutFeedback
                    ...
                    onPress={
                        ()=>{
                            ...
                            this.yelpTest() // Jason testing
                            this.getData('byPref'); //
                            this.getData('recommendation'); //
                            this.getData('test'); //
...
class GroupsAccommodationsPage extends Component {
    ...
    yelpTest = () => {
        ...
        ).then((response) => this.storeData('test', response.data.businesses)); // access fields with rec[0].<key>
    }
    ...
    prefRecommendation = () => {
        ...
        ).then((response) => this.storeData('recommendation', response.data.businesses)); // access fields with rec[0].<key>
